...
import { Search, Phone, Mail, MapPin, ShoppingBag, CheckCircle, XCircle, AlertCircle, Trash2, MessageCircle, Download, Calendar, X } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        ...
        <select onChange={(e) => { if (e.target.value) { downloadLeads(e.target.value === 'all' ? null : e.target.value); e.target.value = ''; } }} style={{ padding: '10px 16px', backgroundColor: '#10b981', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 32 }}>
        ...
        {[
          { key: 'new', icon: AlertCircle, color: '#f59e0b' },
          { key: 'contacted', icon: MessageCircle, color: '#3b82f6' },
          { key: 'converted', icon: CheckCircle, color: '#10b981' },
          { key: 'lost', icon: XCircle, color: '#ef4444' }
        ].map(({ key, icon: Icon, color }) => (
          <div key={key} onClick={() => setFilterStatus(filterStatus === key ? 'all' : key)} style={{ backgroundColor: filterStatus === key ? '#374151' : '#1f2937', borderRadius: 12, padding: 20, cursor: 'pointer', border: filterStatus === key ? `2px solid ${color}` : '2px solid transparent' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <Icon size={16} style={{ color }} />
              <p style={{ fontSize: 13, color: '#9ca3af', textTransform: 'capitalize' }}>{key}</p>
            </div>
            <p style={{ fontSize: 28, fontWeight: 700, color }}>{stats[key]}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input type="text" placeholder="Search by name, phone, or email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchLeads()} style={{ width: '100%', padding: '12px 12px 12px 44px', backgroundColor: '#1f2937', border: '1px solid #374151', borderRadius: 10, color: '#fff', fontSize: 14, outline: 'none' }} />
        </div>
        <button onClick={() => setShowDateFilter(!showDateFilter)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', backgroundColor: (dateFrom || dateTo) ? '#3b82f6' : '#1f2937', border: '1px solid #374151', borderRadius: 10, color: '#fff', fontSize: 14, cursor: 'pointer' }}>
          <Calendar size={16} />
          {dateFrom || dateTo ? `${dateFrom || '...'} to ${dateTo || '...'}` : 'Date Filter'}
        </button>
        {(dateFrom || dateTo) && (
          <button onClick={() => { setDateFrom(''); setDateTo(''); }} style={{ padding: '12px', backgroundColor: '#374151', border: 'none', borderRadius: 10, color: '#ef4444', cursor: 'pointer' }}><X size={16} /></button>
        )}
      </div>
      ...
      {showDateFilter && (
        <div style={{ backgroundColor: '#1f2937', border: '1px solid #374151', borderRadius: 12, padding: 20, marginBottom: 24, display: 'flex', gap: 16, alignItems: 'flex-end' }}>
          ...
          <button onClick={() => { fetchLeads(); setShowDateFilter(false); }} style={{ padding: '10px 20px', backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer' }}>Apply</button>
          <button onClick={() => { setDateFrom(''); setDateTo(''); setShowDateFilter(false); }} style={{ padding: '10px 20px', backgroundColor: '#374151', color: '#9ca3af', border: 'none', borderRadius: 8, cursor: 'pointer' }}>Clear</button>
        </div>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#111827' }}>
              ...
              <th style={{ textAlign: 'left', padding: '16px 24px', fontSize: 12, fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase' }}>Address</th>
          ...
          <tbody>
            {filteredLeads.length === 0 ? (
              <tr><td colSpan="6" style={{ padding: 60, textAlign: 'center' }}><AlertCircle size={48} style={{ color: '#374151', margin: '0 auto 16px' }} /><p style={{ fontSize: 16, color: '#6b7280' }}>No leads found</p></td></tr>
            ) : (
                ...
                return (
                  <tr key={lead._id} onClick={() => setSelectedLead(lead)} style={{ borderTop: '1px solid #374151', cursor: 'pointer' }}>
                    <td style={{ padding: '16px 24px' }}>
                      ...
                      {lead.email && <p style={{ fontSize: 11, color: '#6b7280' }}>{lead.email}</p>}
                    </td>
                    <td style={{ padding: '16px 24px' }}>
                      <p style={{ fontSize: 13, color: '#9ca3af', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{getFullAddress(lead)}</p>
                      {lead.delivery_zone && <p style={{ fontSize: 11, color: '#6b7280' }}>Zone: {lead.delivery_zone}</p>}
                    </td>
                    ...
                    <td style={{ padding: '16px 24px' }}><p style={{ fontSize: 13, color: '#9ca3af' }}>{formatDate(lead.created_at)}</p></td>
                    <td style={{ padding: '16px 24px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-block', padding: '6px 12px', backgroundColor: status.bg, color: status.color, fontSize: 12, fontWeight: 600, borderRadius: 20 }}>{status.label}</span>
                    </td>
                    <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                      <button onClick={(e) => { e.stopPropagation(); deleteLead(lead._id); }} style={{ padding: 8, backgroundColor: '#374151', border: 'none', borderRadius: 6, color: '#ef4444', cursor: 'pointer' }}><Trash2 size={16} /></button>
                    </td>
                  </tr>
                );
              })
          ...
          <div style={{ backgroundColor: '#1f2937', borderRadius: 16, width: '100%', maxWidth: 600, maxHeight: '90vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: 24, borderBottom: '1px solid #374151' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <div style={{ gridColumn: 'span 2' }}><p style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>Full Address</p><p style={{ fontSize: 15, color: '#fff' }}>{selectedLead.address || 'N/A'}</p></div>
              </div>
            </div>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                {Object.entries(statusColors).map(([key, value]) => (
                  <button key={key} onClick={() => updateLeadStatus(selectedLead._id, key)} style={{ padding: '12px 16px', backgroundColor: selectedLead.status === key ? value.bg : '#374151', color: selectedLead.status === key ? value.color : '#9ca3af', border: selectedLead.status === key ? `2px solid ${value.color}` : '1px solid #4b5563', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{value.label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: '0 24px 24px', display: 'flex', gap: 12 }}>
              ...
              <a href={`tel:${selectedLead.phone}`} style={{ flex: 1, padding: 14, backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: 'none', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><Phone size={18} /> Call Now</a>
            </div>
          </div>
        </div>
      )}
