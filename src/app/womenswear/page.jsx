...
function WomenswearContent() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      let url = `/products?category=womenswear&page=${page}&limit=12&sort=${sortBy}`;
  ...
  const updateFilter = (key, value) => {
    ...
    router.push(`/womenswear?${params.toString()}`);
  };
...
export default function WomenswearPage() {
  return (
    <Suspense fallback={<LoadingWomenswear />}>
      <WomenswearContent />
    </Suspense>
  );
}
