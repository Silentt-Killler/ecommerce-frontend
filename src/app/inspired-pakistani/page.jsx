    ...
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, minWidth: isMobile ? 60 : 80, background: 'none', border: 'none', cursor: 'pointer' }}>
      <div style={{ width: isMobile ? 50 : 70, height: isMobile ? 50 : 70, borderRadius: '50%', overflow: 'hidden', border: isActive ? '2px solid #B08B5C' : '2px solid transparent' }}>
        {subcategory.image_url ? (
          ...
          <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg, #E8D5B7 0%, #B08B5C 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function InspiredPakistaniContent() {
  ...
  const updateURL = (filters) => {
    ...
    router.push(`/inspired-pakistani${params.toString() ? '?' + params.toString() : ''}`, { scroll: false });
  };
  ...
  const fetchSubcategories = async () => {
    try {
      const res = await api.get('/subcategories?parent=inspired-pakistani&is_active=true');
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      let url = '/products?category=inspired-pakistani&limit=20';
  ...
  const clearAllFilters = () => {
    ...
    router.push('/inspired-pakistani', { scroll: false });
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FFFFFF', paddingBottom: isMobile ? 90 : 0 }}>
        ...
        <div style={{ backgroundColor: '#FFFFFF', borderBottom: '1px solid #F3F4F6' }}>
          <div style={{ maxWidth: 1600, margin: '0 auto', padding: '16px 40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
              ...
              <span style={{ color: '#0C0C0C', fontWeight: 500 }}>Inspired Pakistani</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ backgroundColor: '#FFFFFF' }}>
        <div style={{ maxWidth: 1600, margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 40px' }}>
          <h1 style={{ fontSize: isMobile ? 24 : 36, fontWeight: 300, letterSpacing: isMobile ? 3 : 6, color: '#0C0C0C', margin: 0, textTransform: 'uppercase', textAlign: isMobile ? 'center' : 'left' }}>Inspired Pakistani</h1>
        </div>
      </div>
...
export default function InspiredPakistaniPage() {
  return <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 40, height: 40, border: '2px solid #B08B5C', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /></div>}><InspiredPakistaniContent /></Suspense>;
}
