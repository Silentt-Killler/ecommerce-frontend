...
function MobileFilterPanel({ isOpen, onClose, subcategories, selectedSubcategory, onSubcategoryChange, selectedSize, onSizeChange, selectedColor, onColorChange, selectedPrice, onPriceChange, onClear }) {
  ...
  const colorOptions = ['Black', 'White', 'Red', 'Blue', 'Pink', 'Purple', 'Yellow', 'Beige', 'Brown', 'Green'];
  const priceOptions = ['Under \E0\A7\B31000', '\E0\A7\B31000 - \E0\A7\B32000', '\E0\A7\B32000 - \E0\A7\B35000', '\E0\A7\B35000 - \E0\A7\B310000', 'Above \E0\A7\B310000'];
    ...
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, minWidth: isMobile ? 60 : 80, background: 'none', border: 'none', cursor: 'pointer' }}>
      <div style={{ width: isMobile ? 50 : 70, height: isMobile ? 50 : 70, borderRadius: '50%', overflow: 'hidden', border: isActive ? '2px solid #B08B5C' : '2px solid transparent' }}>
        {subcategory.image_url ? (
          ...
          <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg, #D4AF37 0%, #B08B5C 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ShoppingBag size={isMobile ? 18 : 24} style={{ color: '#FFFFFF' }} />
          </div>
        )}
      </div>
...
function OriginalPakistaniContent() {
  ...
  const updateURL = (filters) => {
    ...
    router.push(`/original-pakistani${params.toString() ? '?' + params.toString() : ''}`, { scroll: false });
  };
  ...
  const fetchSubcategories = async () => {
    try {
      const res = await api.get('/subcategories?parent=original-pakistani&is_active=true');
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      let url = '/products?category=original-pakistani&limit=20';
      ...
      if (selectedPrice) {
        if (selectedPrice === 'Under \E0\A7\B31000') url += '&max_price=1000';
        else if (selectedPrice === '\E0\A7\B31000 - \E0\A7\B32000') url += '&min_price=1000&max_price=2000';
        ...
        else if (selectedPrice === 'Above \E0\A7\B310000') url += '&min_price=10000';
      }
  ...
  const clearAllFilters = () => {
    ...
    router.push('/original-pakistani', { scroll: false });
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FFFFFF', paddingBottom: isMobile ? 90 : 0 }}>
      ...
      <MobileFilterPanel isOpen={showMobileFilter} onClose={() => setShowMobileFilter(false)} subcategories={subcategories} selectedSubcategory={selectedSubcategory} onSubcategoryChange={handleSubcategoryChange} selectedSize={selectedSize} onSizeChange={handleSizeChange} selectedColor={selectedColor} onColorChange={handleColorChange} selectedPrice={selectedPrice} onPriceChange={handlePriceChange} onClear={clearAllFilters} />
        ...
        <div style={{ backgroundColor: '#FFFFFF', borderBottom: '1px solid #F3F4F6' }}>
          <div style={{ maxWidth: 1600, margin: '0 auto', padding: '16px 40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
              ...
              <span style={{ color: '#0C0C0C', fontWeight: 500 }}>Original Pakistani</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ backgroundColor: '#FFFFFF' }}>
        <div style={{ maxWidth: 1600, margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 40px' }}>
          <h1 style={{ fontSize: isMobile ? 24 : 36, fontWeight: 300, letterSpacing: isMobile ? 3 : 6, color: '#0C0C0C', margin: 0, textTransform: 'uppercase', textAlign: isMobile ? 'center' : 'left' }}>Original Pakistani</h1>
        </div>
      </div>
...
export default function OriginalPakistaniPage() {
  return <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 40, height: 40, border: '2px solid #B08B5C', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /></div>}><OriginalPakistaniContent /></Suspense>;
}
